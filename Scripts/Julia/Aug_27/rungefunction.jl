...
fig, ax = plt.subplots()
ax.plot(x, f, c="olive", ls="dotted", label=L"$\displaystyle \frac{1}{1+25x^{2}}$")
ax.plot(x, fintp, c="teal", label=L"$Interpolation$")
ax.plot(xnodes, fnodes, "m.", label=L"$N = 9$")
ax.set(xlabel=L"$x$", ylabel=L"$\displaystyle \frac{1}{1+25x^{2}}$")
ax.set_title(raw"Runge Function")
